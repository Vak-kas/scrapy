...
class HostSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.divide_file = open('divide2.txt', 'w')
    ...
    def parse(self, response):
        while self.hosts_queue:
            ...
            yield scrapy.Request(url=current_host, callback=self.process_host)

    def process_host(self, response):
        current_host = response.url
        if response.status in [403, 404]:
            self.log(f"Access denied or not found: {current_host}")
